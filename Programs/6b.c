...
int *buf,buf_pos=-1,prod_count,con_count,buf_len; int produce(pthread_t self){ int i = 0;
...
producers = (pthread_t*) malloc(prod_count*sizeof(pthread_t)); printf("Enter the number of Consumers:"); scanf("%d",&con_count);
...
err = pthread_create(producers+i,NULL,&producer,NULL); if(err != 0){

printf("Error creating producer %d: %s\n",i+1,strerror(err)); }else{
...
err = pthread_create(consumers+i,NULL,&consumer,NULL); if(err != 0){
